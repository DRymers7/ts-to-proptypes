...
describe('extractPropsFromParameter', () => {
    ...
    it('should extract primitive props correctly', () => {
        ...
        expect(result).toEqual([
            ...
            {name: 'age', type: {kind: 'primitive', name: 'number'}, required: true},
            {name: 'isActive', type: {kind: 'primitive', name: 'boolean'}, required: true}
        ]);
    });
    ...
    it('should mark optional props correctly', () => {
        ...
        expect(result).toEqual([
            {name: 'required', type: {kind: 'primitive', name: 'string'}, required: true},
            {name: 'optional', type: {kind: 'primitive', name: 'number'}, required: false}
        ]);
    });
    ...
    it('should extract object props correctly', () => {
        ...
        expect(result).toContainEqual({name: 'config', type: {kind: 'object'}, required: true});
        expect(result).toContainEqual({name: 'meta', type: {kind: 'object'}, required: true});
        // Check for nested properties
        expect(result).toContainEqual({name: 'theme', type: {kind: 'primitive', name: 'string'}, required: true});
        expect(result).toContainEqual({name: 'size', type: {kind: 'primitive', name: 'number'}, required: true});
    });
    ...
    it('should extract oneOfType (mixed union) props correctly', () => {
        ...
        if (idProp?.type.kind === 'oneOfType') {
            expect(idProp.type.types).toEqual(expect.arrayContaining([
                {kind: 'primitive', name: 'string'},
                {kind: 'primitive', name: 'number'}
            ]));
        } else {
            throw new Error(`Expected idProp kind to be 'oneOfType', got ${idProp?.type.kind}`);
        }
        ...
        if (contentProp?.type.kind === 'oneOfType') {
            expect(contentProp.type.types).toEqual(expect.arrayContaining([
                {kind: 'primitive', name: 'string'},
                {kind: 'function'},
                {kind: 'array'}
            ]));
        } else {
            throw new Error(`Expected contentProp kind to be 'oneOfType', got ${contentProp?.type.kind}`);
        }
    ...
    it('should handle complex nested props correctly', () => {
        ...
        if (themeProp?.type.kind === 'oneOf') {
            ...
            throw new Error(`Expected themeProp kind to be 'oneOf', got ${themeProp?.type.kind}`);
        }

        // Check for the options prop
        expect(result).toContainEqual({name: 'options', type: {kind: 'object'}, required: true});
